...
s342937\\
email address:\\
{\tt\small s342937@studenti.polito.it}
...
s347156\\
email address\\
{\tt\small s347156@studenti.polito.it}
\and
Mincigrucci Gabriele\\
s358987\\
email address\\
{\tt\small s358987@studenti.polito.it}
}
...
%LogoDet-3K was chosen for three key strategic reasons. First, its scale and diversity, featuring 3,000 brands compared to the few dozen found in classic datasets,
%prevent the model from overfitting to a small closed set.
%Furthermore, the long-tailed distribution of the data perfectly simulates real-world scenarios, making it the ideal candidate for validating Few-Shot
%Learning strategies designed to handle data sparsity. Finally, the contextual complexity of the "in-the-wild" images,
%characterized by cluttered backgrounds, occlusions, and varying lighting, compels the metric learning model to learn robust and invariant features,
%rather than relying on the simple texture matching typical of clean, cropped icons.
LogoDet-3K was selected for three reasons: its scale prevents overfitting compared to smaller datasets; its long-tailed distribution effectively validates Few-Shot Learning in sparse, real-world scenarios; and its complex "in-the-wild" imagery forces the model to learn robust, invariant features rather than simple texture matching.
...
Subsequently, \textbf{Augmentation} is applied during training, transformations, including \texttt{RandomResizedCrop}, \texttt{RandomPerspective}, \texttt{RandomHorizontalFlip}, \texttt{RandomRotation}, \texttt{RandomGrayscale}, and \texttt{ColorJitter}. These techniques simulate the distortions typical of real-world logos (light variations, angled shots, etc.).
...
\begin{table}[H]
   ...
   \begin{tabularx}{\linewidth}{|l|X|}
       ...
       Margin & 0.2 \\
       \hline
       Learning Rate & $1.0 \times 10^{-2}$ (Costant, Factor: 1) \\
       \hline
       Backbone LR & $0.25$ * (Head's LR)\\
       \hline
       Output Norm. & True \\
       \hline
       Embedding & MLP ($2048 \to 1024 \to \text{ReLU} \to 256$) \\
       \hline
       Freeze Strat. & $4 \to 3$ (at epoch 15) \\
       \hline
       Data Aug. & Resize ($224 \times 224$), RandomPerspective ($p=0.3$), RandomHorizontalFlip ($p=0.3$), RandomRotation ($\pm 15^\circ$), RandomGrayscale ($p=0.1$), ColorJitter, Normalize \\
       \hline
   \end{tabularx}
