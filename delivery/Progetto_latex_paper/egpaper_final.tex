...
we also share our pre-processed dataset at: \url{https://drive.google.com/file/d/1yFiHK_r6ae8Xs8uls8F-ccXHPpqVyh74/view?usp=sharing}.
...
The implemented architecture is based on a \textbf{ResNet-50 backbone}, chosen for its optimal balance between computational depth and feature extraction capability. The original model, pre-trained on ImageNet, has been modified to adapt to the \textbf{metric learning paradigm} through a custom \textbf{Embedding Head}. This module consists of a high-dimensional FC layer, followed by \textbf{1D Batch Normalization} and \textbf{Dropout}, ending with a second FC layer for the final latent projection. To ensure the effectiveness of the loss functions, the embeddings undergo \textbf{Spatial Normalization} using the Euclidean norm. This step forces the network to optimize the \textbf{semantic direction} of the logos, ensuring that spatial proximity reflects true \textbf{morphological similarity} rather than scale artifacts.
...
\begin{figure}[H] % Usa [t] per posizionare in alto (meglio di [H] per CVPR)
    \centering
    ...
    % Spazio verticale personalizzabile tra immagine e tabella
    % Modifica questo valore per avvicinare/allontanare (es. 0.2cm, 0.5cm)
    \vspace{0.3cm} 
    
    % --- PARTE TABELLA ---
    % Nota: \captionof richiede il pacchetto 'caption' o 'capt-of' nel preambolo
    \captionof{table}{Configuration hyperparameters related to the results in Figure \ref{fig:41}.}
    \label{tab:config_1}
    
    % Ottimizzazione spazio tabella per CVPR
    \small % Riduce leggermente il font (standard in molte conferenze per le tabelle)
    \renewcommand{\arraystretch}{1.1} % Riduce l'altezza delle righe (era 1.2)
    \setlength{\tabcolsep}{4pt} % Riduce leggermente lo spazio tra le colonne se serve
...
%TABELLA DELL'IMMAGINE NUMERO 4.2 (commentata per evitare problemi di spazio e posizionamento in CVPR)
%\begin{table}[H]
%    \centering
%    \renewcommand{\arraystretch}{1.2}
...
%Tabella dell'immagine numero 4.3 (commentata per evitare problemi di spazio e posizionamento in CVPR)
...
\begin{itemize}
    \item \textbf{Optimizer Selection:} Initial testing on a 10,000-sample subset identified SGD as the more stable choice for this architecture. While Adam introduced significant volatility in the distance threshold during early trials, the SGD baseline (Figure \ref{fig:431}) demonstrates a consistent, stable upward trend in F1-score and a more stable distance threshold. This stability suggests that momentum-based SGD is better suited for maintaining the rigid geometric margin required for this metric learning task.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\myimgwidth]{imm/img432.pdf}
%     \caption{todo}
%     \label{fig:432}
% \end{figure}
...
\textbf{Comparison with Baseline:} While the ResNet-50 baseline achieves high Accuracy, it struggles with retrieval-specific metrics. Our proposed models demonstrate a substantial leap in \textbf{Recall} and \textbf{mAP}, indicating that the Metric Learning approach is more effective at clustering similar logos than standard classification. The most striking difference is the \textbf{J metric}, which rises from $0.3898$ in the baseline to nearly $2.0$, reflecting a more optimized and expansive latent space.
...
The Baseline achieves the highest Accuracy, particularly at $n=5$, while both metric-learning models show a slight decrease, indicating a small trade-off in overall classification correctness. However, Precision improves at $n=1$ for both proposed models and remains competitive at $n=5$, especially for the Contrastive approach. The most significant improvement is observed in Recall, which increases substantially in both Triplet and Contrastive models, with the latter reaching 0.6441 at $n=5$ compared to 0.3493 for the Baseline. As a result, the F1-Score improves consistently, demonstrating that Recall gains are not obtained at the expense of Precision. Overall, these variations indicate that metric learning enhances retrieval effectiveness despite a marginal reduction in Accuracy.
